...
module.exports.authentication = async (request, response, next) => {
  ...
  try {
    ...
    if (!authHeader) {
      return response.status(400).send('Acesso negado: usuário não autenticado.')
    }

    const [header, token] = authHeader.split(' ')

    if (String(header).toLowerCase() !== 'bearer') {
      return response.status(400).send('Acesso negado: tipo de autorização incorreta.')
    }
    ...
    if (request.originalUrl === '/auth/tokens/refresh') {
      ...
      if (!(isValidJWT)) return response.status(400).send('Acesso negado: token inválido')
    ...
    jwt.verify(token, AUTH_PRIVATE_KEY, async (error, result) => {
      try {
        if (error) return response.status(400).send('Acesso negado: token expirada.')

        if (!result) return response.status(400).send('Acesso negado: token inválida.')
